...
function loginvalidate(){ // 로그인 유효성 검사
    ...
    if(inputEmail.value.trim().length == 0){
        ...
        return false; // 기본이벤트 제거를 위해 false를 반환
    }
    // 비밀번호를 입력하지 않은 경우 false 반환
    if(inputPw.value.trim() == ""){

        alert("비밀번호을 입력해주세요.");
        inputPw.value =""; // 비밀번호 input 입력된 내용을 모두 삭제
        inputPw.focus(); // 비밀번호 input에 포커스를 맞춰줌
        return false;
    }
...
function selectAll(){ // 회원 전체 조회 함수
    ...
    // ajax 코드 
    $.ajax({
        // 현재 주소 : /community/member/selectAll
