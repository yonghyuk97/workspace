...
const deleteSet = new Set(); // 순서x, 중복 허용x (여러번 클릭 시 중복 값 저장 방지)
...
for(let i=0; i < inputImage.length; i++){
    ...
    inputImage[i].addEventListener("change", function(){
        ...
        if(this.files[0] != undefined){ // 파일이 선택되었을 때
            ...
            reader.onload = function(e){ // reader가 파일을 다 읽어온 경우
                ...
                // 이미지가 성공적으로 불러와 졌을 때
                // deleteSet에서 해당 레벨을 제서(삭제 목록에서 제외)
                deleteSet.delete(i);
            }
    ...
    deleteImage[i].addEventListener("click", function(){
        ...
        // 미리보기가 존재하는 경우에만  (이전에 추가된 이미지가 있을 떄만 ) x 버튼 동작
...
function writeValidate(){
    ...
    // -> Js 배열 특징 사용
    // --> JS 배열을 HTML요소 또는 console에 출력하게 되는 경우 1,2,3 같은 문자열로 출력됨
    //      (배열 기호가 벗겨짐)
