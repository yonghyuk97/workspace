    ...
    $.ajax({
        ...
        success : function(rList){
            ...
            for(let reply of rList){
                ...
                // 작성자 영역(p)에 프리필, 닉네임, 작성일 마지막 자식으로 추가
...
addReply.addEventListener("click", function(){ // 댓글 등록 버튼이 클릭 되었을 떄

    // 1) 로그인이 되어 있나? -> 전역변수 loginMemberNo를 이용   
...
function deleteReply(replyNo){
    ...
    if( confirm("정말로 삭제 하시겠습니까?") ){
        ...
        // 전달방식 : "get"
        // success : 삭제 성공 시 --> alert 삭제되엇습니다. 출력
        //                      댓글 목록 비동기 조회 함수 호출
        //           삭제 실패 시 --> alert로 삭제 실패 출력
        // error : 앞 error 코드 참고
...
function showUpdateReply(replyNo, btn){
    ...
    if(temp.length > 0){ // 수정이 한개 이상 열려 있는 경우
    ...
    // 이것도 가능
    // let beforeContent = btn.parentElement.previousElementSibilng.innerHTML;
    //                      버튼의 부모(div)의 이전형제의 innerHTML;
...
function updateCancle(btn){
    // 매개변수 btn : 클릭된 취소 버튼
    // 전역 번수 beforeReplyRow : 수정 전 원래 행(댓글)을 저장한 변수
