...
(function(){
    ...
    if(goToListBtn != null){// 목록으로 버튼이 화면에 있을 떄만 이벤트 추가
        ...
        goToListBtn.addEventListener("click", function(){
            ...
            if(params.get("cp") != ''){ // 쿼리스트링에 cp가 있는 경우
...
(function(){
    ...
    if(deleteBtn != null){ // 이 버튼이 화면에 존재할 때
        deleteBtn.addEventListener("click", function(){
            // 현재 : /community/board/detail?no=1562&cp=1&type=1

            // 목표 : /community/board/delete?no=1562&type=1

            let url ='delete'; // 상대경로 형식으로 작성

            // 주소에 작성된 쿼리스트링 필요한 파라미터만 얻어와서 사용

            // 1) 쿼리스트링에 준재하는 파라미터 모두 얻어오기
            const params = new URL(location.href).searchParams;
...
(function(){
    ...
    if(select != null){ // 검색창 화면이 존재할 때만 코드 적용
        ...
        // option을 반복 접근해서 value와 key와 같다면 selectd 속성 추가
