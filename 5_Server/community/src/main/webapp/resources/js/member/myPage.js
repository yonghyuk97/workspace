...
function infoValudate(){
    ...
    // 010 011 016 017 019
    // 02 031 032 041 042 051 052 070

    if(memberNickname.value.trim().length == 0){ // 미작성 시 : 닉네임을 입력해주세요.
        alert("닉네임을 입력해주세요.");
    ...
    // 전화번호 유효성 검사
    if(memberTel.value.trim().length == 0){ // 미작성 시 : 전화번호를 입력해주세요. (-제외)
        alert("전화번호을 입력해주세요. (-제외)");
        memberTel.focus();  
        return false;  
    }
...
function changePwValidate(){
    ...
    if(newPwConfirm.value.trim().length == 0){
        return printAlert(newPwConfirm, "새 비밀번호 확인를 입력해주세요.");
    }
