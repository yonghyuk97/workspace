...
<body>
	...
	<pre>
        JSP에서 자주 사용되거나 공통적을 사용되는 Java 코드를
	...
	<h3>JSTL 사용을 위한 선언</h3>

	<pre>
        JSTL을 사용하고자 하는 JSP가 있을 경우
        해당 JSP 최상단에 JSTL 라이브러리를 추가하는 지시자 Tag Library를 작성해야 한다.
        
        prefix : 접두사, 다른 태그와 구별할 수 있는 namespace
        		 (태그 앞에 붙은 태그명)
        		 
        url(Uniform Resource Lcator) : 인터넷에서 특정 자원 위치 지정(주소) 
	...
	<pre>
		...
		- scope 선택 가능
		- scope 미 작성시 모든 scope에서 일치하는 변수명 모두 제거
	...
	<pre>
		...
		- if문만 가능하고 else는 불가능(c:else 존재하지 X)
	...
	<c:if test="${ temp > 20 }">
		<!-- test에 작성된 조건이 만족(true)일 때만 화면에 출력  -->
		temp는 20보다 크다
	</c:if>
