...
public class TCPServer {
	...
	//		 순차적으로 데이터를 전달하고 확인 및 오류 시 재전송 해준다.
	...
	public void serverStart() {
		//TCP socket 프로그래밍 순서
//		1. 서버의 포트번호 정함
		int port = 8500; // port번호는 0~65535 사이에 지정이 가능하다.(암기)
						// 단, 1023번 이하는 이미 사용중인 경우가 많으니 제외(암기)
		
		//* 사용할 변수를 미리 선언 * <선언방법 암기>
		ServerSocket serverSocket = null; // 서버 소켓 저장 변수
		Socket clientSocket = null; // 클라이언트 소켓 저장 변수
		
		InputStream is = null; // 클라이언트 에서 서버 입력용 스트림 변수
...
//		4. 접속 요청이 오면 요청 수락 후 해당 클라이언트에 대한 소켓 객체 생성
		// -> 요청을 수락하면 자동으로 Socket 객체가 얻어와진다.	
			...
			String clientIP = clientSocket.getInetAddress().getHostAddress();//인터넷주소를 얻어온다.
...
//		6. 보조 스트림을 통해 성능 개선
			...
			pw = new PrintWriter(os); // 연결할 떄 사용해준다.
...
//		7-1) 서버 -> 클라이언트에게 출력(메세지 전송)	
			...
			String time = sdf.format(now); // now에 저장된 시간 포멧을 변경한다.
...
//		8. 통신 종료
	...
	//	사용한 스트림, 소켓, 자원을 모두 반환(close)
			...
			try {
				//보조스트림 close시 연결된 기반스트림(is, os 도 같이 close 된다.
				if(pw != null) 		pw.close();
				if(br != null)		br.close();
				...
				// if 문에 {}를 사용 안하면 다음 한 줄(세미콜론 기준)이 if문 내부 코드가 된다.
