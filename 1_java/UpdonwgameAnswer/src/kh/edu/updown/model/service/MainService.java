	    	  ...
	    	  while(true) {
	    		  ...
	    		  for(int i = 0; i<members.length; i++) {
	    			  if(members[i] != null) { // members 배열 요소가 null이 아닌경우 == 회원 정보가 있는 경우
	    				  ...
	    				  if(memberId.equals(members[i].getMemberName())){
	    			  ...
	    			  } else { // members 배열 요소가 null 인 경우 == 회원 정보가 없는 경우
	   ...
	   public void login() {
	      ...
	      for(int i =0; i < members.length; i++) {
	    	  ...
	    	  if(members[i] != null) { // members 배열 요소가 null이 아닌경우 == 회원 정보가 있는 경우
	    		  ...
	    		  if(members[i].getMemberId().equals(inputId) &&
	    				  members[i].getMemberPw().equals(inputPw)) {
	    			  
	    			  System.out.println(members[i].getMemberName() + "님 환영합니다.");
	    			  
	    			  loginMember = members[i]; // loginMember 필드에 일치하는 회원 객체의 주소를 얕은복사 
	    		  }
	      ...
	      // 아이디, 비밀번호를 비교한 후에도 loginMember 필드가 null == 로그인 실패
	      if(loginMember == null) {
