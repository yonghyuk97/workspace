...
테이터 딕셔너리는 사용자가 테이블을 생성하거나 사용자를 변경하는 등의
작업을 할 때 데이터베이스 서버의 의해 자동으로 갱신되는 테이블

- User_tables : 자신의 게정이 소유한 객체 등에 관한 정보를 조회 할 수 있는 딕셔너리 뷰
...
-- MEMBER 테이블 생성
CREATE TABLE MEMBER(
   MEMBER_ID VARCHAR2(20), -- 가변 길이 문자열 20바이트(영어,숫자 만 작성시 20글자)
...
-- 만든 테이블 확인
SELECT * FROM MEMBER;
SELECT * FROM USER_TABLES; -- 딕셔너리 뷰( 데이터 딕셔너리에서 일부분만 뽑아서 만든 가상의 테이블)

-- 접속창 -> 계정열고 -> 테이블 클릭 -> 위쪽에 새로고침 클릭 -> MEMBER 테이블 보임
...
-- 주민등록 번호 데이터 타입을 NUMBER로 지정할 경우 문제점
...
/*
   ...
   데이터 무결성 보장을 목적으로 함
   -- > 데이터 무결성이란? 중복 데이터 최소화 , NULL 최소화 X 지향 
...
CREATE TABLE USER_USED_NN(
     USER_NO NUMBER NOT NULL, -- 사용자 번호 (무조건 있어야 함 == NULL X ) 
                            --> 컬럼 레벨 제약조건 설정
     USER_ID VARCHAR2(20),
...
-- ORA-01400: NULL을 ("LHK"."USER_USED_NN"."USER_NO") 안에 삽입할 수 없습니다
...
-- 단, UNIQUE 제약 조건이 설정된 컬럼에 NULL 값은 중복이 삽입 가능

-- UNUQUE 제약 조건 테이블 생성
CREATE TABLE USER_USED_UK(
     USER_NO NUMBER,
     --USER_ID VARCHAR2(20) CONSTRAINT USER_ID_U UNIQUE, -- 제약 조건명 지정
...
-- ORA-00001: 무결성 제약 조건(LHK.USER_ID_U)에 위배됩니다
     --> UNIQUE 
...
-- ORA-00001: 무결성 제약 조건(LHK.USER_ID_NAME_U)에 위배됩니다

SELECT * FROM USER_USED_UK2;

--------------------------------------------------------------------------------

-- 3. PRIMARY KEY(기본기) 제약조건

-- 테이블에서 한 행의 정보를 찾기위해 사용할 칼럼을 의미함
-- 테이블에 대한 *식별자(IDENTIFIER)* 역할을 함
-- NOT NULL + UNIQUE 제약조건을 의미
-- 한 테이블 당 한 개만 설정할 수 있음
-- 컬럼레벨, 테이블 레벨 둘다 설정 가능함
...
--> 기본키 중복으로 오류
-- ORA-00001: 무결성 제약 조건(LHK.USER_NO_PK)에 위배됩니다
INSERT INTO USER_USED_PK
VALUES(NULL, 'user03', 'pass03', '유관순', '여', '010-9999-3131', 'yoo123@kh.or.kr');
--> 기본키가 NULL 이므로 오류
-- ORA-01400: NULL을 ("LHK"."USER_USED_PK"."USER_NO") 안에 삽입할 수 없습니다
...
--ORA-00001: 무결성 제약 조건(LHK.PK_USERNO_USERID)에 위배됩니다
...
CREATE TABLE USER_USED_FK(
  ...
  PHONE VARCHAR2(30),
  EMAIL VARCHAR2(50),
  GRADE_CODE NUMBER --CONSTRAINT GRADE_CODE_FK REFERENCES USER_GRADE(GRADE_CODE) -- 컬럼 PK 설정
  , 
...
-- ORA-02291: 무결성 제약조건(LHK.GRADE_CODE_FK)이 위배되었습니다- 부모 키가 없습니다
             --> 참조 무결성 위배
...
WHERE GRADE_CODE = 10;
-- ORA-02292: 무결성 제약조건(LHK.GRADE_CODE_FK)이 위배되었습니다- 자식 레코드가 발견되었습니다

-- 1) ON DELETE RESTRICTED(삭제 제한)로 기본 지정되어 있음
-- FOREIGN KEY로 지정된 칼럼에서 사용되고 있는 값일 경우 
-- 제공하는 컬럼의 값은 삭제하지 못함

-- GRADE_CODE 중 20은 외래키는 참조되고 있지 않으므로 삭제가 가능함
COMMIT;
...
-- > ON DELETE SET NULL 옵션이 설정 되어있어 오류 없이 삭제됨
...
-- 3) ON DELETE CASECADE : 부모키 삭제시 자식키도 함께 삭제됨
...
-- GENDER 컬럼에 CHECK 제약조건으로 '남' 또는 '여'만 기록 가능한데 '남자' 라는 조건 이외의 값이
-- 들어와서 에러 발생 
-- ORA-02290: 체크 제약조건(LHK.GENDER_CHECK)이 위배되었습니다
...
-- NOT NULL 제약 조건만 복사면 EMPLOYEE_COPY 테이블에
-- EMP_ID 컬럼에 PRIMIARY KEY 제약조건 추가
...
CONSTRAINT FK_DEPT_CODE FOREIGN KEY(DEPT_CODE) REFERENCES DEPARTMENT;
