...
MODIFY DEPT_TITLE NOT NULL; -- NOT NULL 은 MODIFY사용 !! (꼭 기억)

--------------------------------------------------------------------------------

-- DELT_COPY에 추가한 제약조건 중 PK 빼고 모두 삭제
ALTER TABLE DEPT_COPY DROP CONSTRAINT DEPT_TITLE_U;
ALTER TABLE DEPT_COPY DROP CONSTRAINT LOCATION_ID_CHK;
ALTER TABLE DEPT_COPY DROP CONSTRAINT SYS_C007506; -- NOT NULL

-- NOT NULL 제거 시 MODIFY 사용
ALTER TABLE DEPT_COPY 
MODIFY DEPT_TITLE CONSTRAINT SYS_C007507 NULL;
...
--ALTER TABLE  테이블명 ADD(컬렴명 데이터타입 [DEFAULT '값']);
...
SELECT * FROM DEPT_COPY; -- LNAME 컬럼이 추가되어 있고 , 컬럼 값이 모두 '한국'으로 되어있다

-- (수정)
-- DEPT_COPY 테이블의 DEPT_ID 컬럼의 테이터 타입을 CHAR(2) -> VARCHAR2(3) 으로변경
...
--  ALTER TABLE 테이블명 MODIFY 컬렴명 DEFAULT '값';
...
ROLLBACK; -- 트랜잭션(DML을 이용한 데이터 변경 사항을 저장)을 삭제하고 마지막 커밋상태로 돌아감
...
SELECT * FROM DEPT_COPY;
ROLLBACK; -- 트랙잭션에서 'D0' INSERT 내용을 삭제
SELECT * FROM DEPT_COPY;
INSERT INTO DEPT_COPY VALUES('D0'); -- 'D0'삽입
SELECT * FROM DEPT_COPY;
...
-- 해결 방법 1 : 자식 테이블 삭제 -> 부모 테이블 순서로 삭제하기
-- (참조하는 테이블이 없으면 삭제 가능)
DROP TABLE TB2;
DROP TABLE TB1;
