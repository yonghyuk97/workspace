...
-- [관리자 계정] member 계정 생성
CREATE USER cyh_member IDENTIFIED BY member1234;

-- [관리자 계정] CONNECT, RESOURCE + CREATE VIEW 권한 부여 + 객체 생성 공간 할당
GRANT CONNECT,RESOURCE, CREATE VIEW TO cyh_member;
ALTER USER cyh_member DEFAULT TABLESPACE SYSTEM QUOTA UNLIMITED ON SYSTEM;

-- cyh_member 계정 접속 방법 추가
...
CREATE TABLE MEMBER(
    ...
    MEMBER_NN VARCHAR2(30) NOT NULL,
...
COMMENT ON COLUMN MEMBER.MEMBER_NN IS '회원 이름';
...
SELECT B.BOARD_NO, BOARD_TITLE, CREATE_DATE, READ_COUNT, MEMBER_NN,
...
ORDER BY REPLY_NO ASC; -- 최근 댓글이 하단
...
-- ORA-02292: 무결성 제약조건(CYH_MEMBER.SYS_C007547)이 위배되었습니다- 자식 레코드가 발견되었습니다
...
-- 제약조건은 ALTER 변경이 없다 !!! -> 삭제 후 다시 추가
ALTER TABLE REPLY DROP CONSTRAINT SYS_C007547;
...
-- 게시글 작성
INSERT INTO BOARD 
VALUES(SEQ_BOARD_NO.NEXTVAL, ?(제목), ?(내용), DEFAULT, DEFAULT, ?(작성자));

-- 게시글 검색 
SELECT BOARD_NO, BOARD_TITLE, CREATE_DATE, READ_COUNT, MEMBER_NN,
    (SELECT COUNT(*) FROM REPLY R
    WHERE R.BOARD_NO = B.BOARD_NO) REPLY_COUNT
FROM BOARD B
JOIN MEMBER USING(MEMBER_NO)
-- WHERE BOARD_TITLE LIKE '%' || ? || '%'; -- 제목 검색
-- WHERE BOARD_TITLE LIKE '%' || ? || '%'; -- 내용검색

--WHERE BOARD_TITLE LIKE '%' || ? || '%' 
--OR BOARD_CONTENT LIKE '%' || ? || '%' -- 제목 + 내용

WHERE MEMBER_NN LIKE '%' || ? || '%' -- 작성자 검색
