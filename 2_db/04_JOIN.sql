...
- 관계형 데이터베이스는 최소한의 데이터를 테이블에 담고 있어
  ...
  두 테이블에서 같은 데이터를 저장한느 컬럼이 연결고리가 됨
...
-- EMPLOYEE 테이블과 DEPARTMNET 테이블의 연결할 때
...
-- 작성 방법 크게 ASNI 구문과 오라클 구문으로 나뉘고
-- ANSI에서 USING 과 ON을 쓰는 방법으로 나뉜다.

-- * ANSI 표문 구문
-- ANSI 미국 국립 표준 협회를 뜻함 , 미국의 산업표준을 제정하는 민간단체로
-- 국제표준화기구 ISO에 가입되어있다.
...
-- 오라클 
SELECT EMP_ID,EMP_NAME,DEPT_CODE,DEPT_TITLE
FROM EMPLOYEE, DEPARTMENT -- FROM 절에 사용할 테이블을 모두 작성
...
--ANSI
-- 연결에 사용할 컬럼명이 같은 사용 USING(컬럼명)을 사용함
...
-- 조인되는 테이블의 각 행등의 모두 매핑된 데이터가 검색되는 방법(곱집합)
...
-- 조인 순서를 지키지 않는 경우(에러발생)
SELECT EMP_NAME, DEPT_TITLE,LOCAL_NAME
FROM EMPLOYEE
JOIN LOCATION ON(LOCAL_CODE = LOCATION_ID) -- ORA-00904: "LOCATION_ID": 부적합한 식별자
JOIN DEPARTMENT ON(DEPT_CODE = DEPT_ID);
...
FROM EMPLOYEE , DEPARTMENT , LOCATION
WHERE DEPT_CODE = DEPT_ID AND LOCATION_ID = LOCAL_CODE AND BONUS IS NOT NULL;
-- 8명
