...
- 데이터를 조회 (SELECT) 하면 조건에 맞는 행동이 조회됨
  ...
  - RESULT SET 에는 0개 이상의 행이 포함될 수 있다.
...
-- 조회되는 결과 칼럼 값에 산술 연산이 반영된다.

-- EMPLOYEE 테이블에서 모든 사원의 사번, 이름, 급여 , 급여 + 100만 조회
...
-- SYSDATE : 시스템상의 현재 날짜
--           ( 년,월,일,시,분,단위 까지 표현 가능하지만,
--           디펠로퍼의 날짜 표기 방법이 년/월/일 로 지정되어 있는 것이다.)

-- DUAL(DUmmy tAbLe) : 가짜 테이블(원시 테이블, 단순 조회 테이블)

-- ** DB 날짜 데이터의 연산(+,-) 이 가능하다 (일 단위) **

SELECT SYSDATE ,SYSDATE +1 ,SYSDATE-1 FROM DUAL;

-- EMPLOYEE 테이블에서 이름, 입사일, 오늘까찌 근무한 날짜 조회
...
-- SESELECT 조회 결과의 집합인 RESULT SET에 컬럼명을 지정

/*
 1) 컬럼명 AS 별칭 : 띄어쓰기X , 특수문자X , 문자O
 2) 컬럼명 별칭 : 1) 번에서 AS만 생략 한 것
...
-- 컬럼명 BETWEEN A AND B : 컬럼 값이 A이상 B이하가 아닌 경우
--                      == 컬럼 값이 A미만, B초과인 경우
...
/*** '1990/01/01' 날짜를 문자열 형식으로 작성하게 되면
      DB가 알아서 판단하여 날짜 타입(DATE)으로 형변환을 진행함. ***/
...
'%' 예시
1) 'A%' : 문자열이 A로 시작하는 모든 칼럼 값
2) '%A' : 문자열이 A로 끝나는 모든 칼럼 값
3) '%A%' : 문자열에 A가 포함되어 있는 모든 칼럼 값
...
-- EMPLOYEE 테이블에서 
-- 전화번호가 010으로 시작하는 사원의 사번, 이름 전화번호조회
...
-- 사번이 200,205,210인 아닌(NOT)사원의 사번,이름 조회
SELECT EMP_ID,EMP_NAME
FROM EMPLOYEE
...
SELECT EMP_NAME || '의급여는' || SALARY || '원 입니다.' AS 결과
FROM EMPLOYEE;
...
-- EMPLOYEE 테이블명에서 전화번호가 없는 사원의 사번,이름,전봐번호 조회
...
-- ORDER BY BONUS DESC;  -- NULLS FURST 기본값
