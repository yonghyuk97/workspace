...
-- EMPLOYEE 테이블에서
-- 모든 사원의 사번,이름,부서명,직급 조회

SELECT EMP_ID,EMP_NAME,DEPT_TITLE,JOB_NAME
FROM EMPLOYEE
LEFT JOIN DEPARTMENT ON (DEPT_CODE = DEPT_ID)
JOIN JOB USING(JOB_CODE);
-- > 자주 사용하긴 하지만 매번 쓰기 힘들다... -> VIEW를 생성
...
-- (실행 시 관리자 계정으로 할 것)
GRANT CREATE VIEW TO lhk; -- Grant을(를) 성공했습니다.
...
-- ORA-01400: NULL을 ("LHK"."DEPT_COPY2"."DEPT_TITLE") 안에 삽입할 수 없습니다

-- VIEW를 이용한 INSERT 시 원본 테이블에 삽입이 된다.
-- 원본테이블 삽입 시 VIEW INSERT 구문이 미포함된 컬럼에는 NULL 이 저장된다.
-- 그런데 DEPT_TITLE 컬럼에 NOT NULL 제약조건이 설정되어 있음
--> 오류가 발생! VIEW를 이용한 INSERT 실패

-- 결론 : VIEW 가지고 DMP 웬만하면 하지마세요..
 ...
 *** SEQUENCE를 사용하는 방법 : PRIMARY KEY 컬럼에 사용될 값을 생성하는 용도로 사용
  ...
  ** 시퀀스 사용 방법 **
  ...
  1) 시퀀스명.NEXTVAL : 다음 시퀀스 번호를 얻어옴 (INCREMNET BY만큼 증가된 값)
...
-- EMPLOYEE_COPY4의 EMP_ID 칼럼에 PK 제약조건 추가
...
START WITH 223
INCREMENT BY 5;
-- Sequence SEQ_EMP_ID이(가) 생성되었습니다.

-- NEXTVAL 호출 없이 CURRVAL 호출을 하면 발생하는 오류 확인
...
-- NEXTVAL 호출2
SELECT SEQ_EMP_ID.NEXTVAL FROM DUAL; -- INCREMENT BY 만큼 증가한 값 조회
-- 228

-- CURRVAL 호출
SELECT SEQ_EMP_ID.CURRVAL FROM DUAL; -- NEXTVAL 호출 후 CURRVAL 호출 시 오류 없음
...
SELECT SEQ_EMP_ID.CURRVAL FROM DUAL; --- 증가하지않고 그대로 적용
...
[작성법]
  ...
  * 시퀀스를 잘못 다루어 번호가 공백이 있어서 다시 처음부터 시작하고 싶은 경우
  --> 시퀀스 삭제 후 재생성  ( 수정불가하므로 재생성 해야한다 )
*/  

-- SEQ_EMP_ID의 증가값을 5 -> 1로 변경
...
* 인덱스 단점
 - 데이터 변경(UPDATE , INSERT , DELETE ) 작업 빈번한 경우 오히려 성능이 저하되는 문제가 발생
 - 인덱스도 하나의 객체이다보니 이를 저장하기 위한 별도 공간이 필요하다 보니
 - 인덱스 생성 시간이 필요
...
WHERE TEST_ID = 'TEST500000'; -- 0.013초
