...
-- 함수는 SELECT절 , WHERE절 , GROUP BY , HAVING 사용 가능
...
-- 지정한 위치부터 지정한 순번째로 검색되는 시작 위치를 반환

-- 문자열에서 맨 앞에 있는 B의 위치 조회
SELECT INSTR('AABBAACAABBAA' , 'B') AS FROM DUAL; 

-- 문자열에서 5번째 부터 검색해서 맨 앞에있는 B 위치 조회
SELECT INSTR('AABAACAABBAA' , 'B', 5) AS FROM DUAL;
...
-- 주어진 컬럼이나 문자열의 앞, 뒤, 양쪽에 있는 지정된 문자를제거
-- > (보통 양쪽 공백 제거에 많이 사용)
...
SELECT 123.456, ROUND(123.456, 1) FROM DUAL; -- 소수점 첫째 자리 자리까지 출력 == 소수점 둘째 자리 반올림
...
SELECT ROUND(MONTHS_BETWEEN(SYSDATE,'2023/04/14')) || '개월' AS 수강기간 FROM DUAL;

-- EMPLOYEE 테이블에서
-- 사원의 이름, 입사일, 근무 개월 수 , 근무 햇수 조회
SELECT EMP_NAME, HIRE_DATE,
'근무' ||CEIL(MONTHS_BETWEEN(SYSDATE,HIRE_DATE)) || '개월 차' "근무 개월 수",
'근무' ||CEIL(MONTHS_BETWEEN(SYSDATE,HIRE_DATE) / 12) || '년차 ' "근무 햇수"
FROM EMPLOYEE;
...
SELECT SYSDATE, TO_CHAR(SYSDATE,'DY AM HH:MI:SS') FROM DUAL;
...
--> 기존에 없던 패턴 추가 시 ""(쌍타옴표)로 감싸줘서 문자열 그대로를 출력하게함
...
SELECT TO_CHAR(TO_DATE('041030 143000' , 'YYMMDD HH24MISS'), 'YYYY/MM/DD HH24"시"MI"분"') 
FROM DUAL;
...
/* 숫자 형변환 */
-- TO NUMBER(문자데이터, [포멧]) : 문자형 데이터를 숫자 데이터로 변경
...
-- NVL(컬렴명,컬럼값이 NULL 일때 바꿀 값) : NULL인 컬럼값을 다른 값으로 변경
...
SELECT EMP_NAME,JOB_CODE,TO_CHAR((SALARY+(SALARY * NVL(BONUS,0.1))) * 12, 'L999,999,999') AS "연봉(원)"
FROM EMPLOYEE;
...
SELECT EMP_NAME, DEPT_CODE,
    ...
    EXTRACT(YEAR FROM TO_DATE(SUBSTR(EMP_NO,1,6),'RR/MM/DD')) "나이"
FROM EMPLOYEE;
...
-------------------------------------------------------------------------
/* 그룹 함수 */
-- 하나 이상의 행을 글부으로 묶어 연산하여 총합, 평균 등의 하나의 결과 행으로 반환하는 함수
