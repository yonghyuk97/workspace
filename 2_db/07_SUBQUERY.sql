...
-- 3) 전 직원의 평균 급여보다 만흥 급여를 받고 있는 직원 조회
--> 위의 2단계르 하나의 쿼리로 가능하다 --> 1) 쿼리문을 서브쿼리로 !!
...
-- 1. 단일행 서브쿼리 (SINGLE ROW SUBQUETY)
...
-- 1) 부서별 급여 합 중 가장 큰값 조회 (부서별 급여 합 1등)
...
UNION -- 합집합 ( 2개의 RESULT SET을 하나로 합칩)
...
-- 3) 대리 직급의 직원들 중에서 과장 직급의 최소 급여보다 많은 받는 직원
...
-- 차장 직급 급여의 가장 큰 값보다 많은 받는 과장 직급의 직원
-- 사번, 이름, 직급, 급여를 조회하세요
...
-- 2) DEPARTMENT 테이블에서 위의 결과와 동일한 LOCATION_ID와 동일한 DEPT_ID 조회
SELECT DEPT_ID
FROM DEPARTMENT
...
-- 3) 최종적으로 EMPLOYEE 테이블에서 위에 결과들과 동일한 DEPT_CODE를 가지는 사원 조회
SELECT EMP_NAME,DEPT_CODE
FROM EMPLOYEE
WHERE DEPT_CODE IN (SELECT DEPT_ID  -- 다중행 일때 IN 사용해야함
                   FROM DEPARTMENT
                   WHERE LOCATION_ID = (SELECT LOCAL_CODE
                     FROM LOCATION 
                     WHERE NATIONAL_CODE = 'KO'));
...
-- 1) 급여를 200, 600만 받는 직원 (200만, 600만 평균 급여라 생각 할 경우)
SELECT EMP_ID,EMP_NAME,JOB_CODE,SALARY
FROM EMPLOYEE
...
-- 5. 상[호연]관 서브쿼리 ( 메인쿼리 1행씩 우선 해석, 서브커리 나중에 해석)
...
SELECT EMP_NAME,SALARY,JOB_CODE,(SELECT FLOOR(AVG(SALARY)) FROM EMPLOYEE SUB 
                                 WHERE SUB.JOB_CODE = MAIN.JOB_CODE) AS 평균
FROM EMPLOYEE MAIN;
...
SELECT EMP_ID,EMP_NAME,MANAGER_ID,NVL((SELECT MANAGER_ID FROM EMPLOYEE E2 
                                  WHERE E1.MANAGER_ID = E2.EMP_ID),'없음')
FROM EMPLOYEE E1;
...
FROM EMPLOYEE
ORDER BY SALARY DESC;
-->  해결 1의 조회 결과 (RESULT SET)을 가상의 테이블(== VIEW) 취급을 할 예정
...
-- 급여 평균이 3위 안에 드는 부서의 부서코드와 부서명, 평균 급여를 조회
...
-- DANSE_RANK() OVER() : 동일한 순위 이후의 등수를 이후의 순위로 계산
