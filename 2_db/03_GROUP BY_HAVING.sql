...
-- 여러 개의 결과 값을 산출하기 위해 그룹 함수가 적용된 그룹의 ORDER BY절에 기술하여 사용

-- EMPLOYEE 테이블에서 부서코드, 부서(그룹) 별 급여 합계 조회
SELECT DEPT_CODE, SUM(SALARY)
FROM EMPLOYEE;
--> DEPT_CODE 컬럼은 23행,SUM(SALARY) 칼럼은 1행
-- 하나의 표에 작성할 수 없음
...
-- EMPLOYEE 테이블에서
-- 부서코드,부서 별 급여의 합계, 부서 별 급여의 평균(정수처리), 인원 수 조회하고
-- 부서 코드 순으로 정렬
...
SELECT DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') 성별,
     ...
     COUNT(*) AS "인원 수"
FROM EMPLOYEE
GROUP BY DECODE(SUBSTR(EMP_NO,8,1),'1','남','2','여') -- 별칭 사용 불가
ORDER BY "인원 수" DESC;
 
 -- * WHERE절 GPOUP BY절을 혼합하여 사용
 --> WHERE절은 각 컬럼 값에 대한 조건 (SELECT문 해석 순서를 잘 기억하는것이 중요 !!)
 ...
 -- * 여러 컬럼을 묶어서 그룹으로 지정 가능 -- > 즉 그룹내 그룹 이 가능하다!
 -- *** GROUP BY 사용시 주의사항
 ...
 -- EMPLOYEE 테이블에서 부서 별로 같은 직급인 사원의 급여 합계 조회하고
 -- 부서 코드 오름차순으로 정렬
...
FROM EMPLOYEE --1
WHERE SALARY >= 3000000 --2 급여가 300만 이상인 직원의 부서별 급여 평균
GROUP BY DEPT_CODE  --3
ORDER BY DEPT_CODE; --5
-- EMPLOYEE 테이블에서 급여가 3000000원이상 인 것을 부서별 그룹으로 묶어주고
-- 조회를 부서코드,급여평균 오름차순으로 나오게 한다
...
-- 부서별 그룹의 급여 합계 중 9백만원을 초과하는 부서코드의 급여 합계 조회
-- 부서 코드 순으로 정렬
SELECT DEPT_CODE,SUM(SALARY)
FROM EMPLOYEE
GROUP BY DEPT_CODE
...
-- 여러가지 조건을 있을 때 그의 해당하는 여러개의 결과값을 결합시키고 싶을 때 사용
-- 초보자들이 사용하기 쉽다.
-- (주의) 집합 연산에 사용되는 SELECT 문은 SELECT 절이 동일해야함
