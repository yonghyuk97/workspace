...
public class MemberView { // 회원 관련 화면 입/출력
	...
	public void signup() { 
		...
		try {
			...
			while(true) { // 중복 아이디가 없을 경우 종료하는 if문 필요
				...
				// 아이디 중복 검사 (DB에 일치하는 아이디가 있으면 "중복" --> 아이디 다시 입력받기)
				
				int result = service.duplicateCheck(memberId);
				// result 는 중복이면 1, 아니면 0 반환될 예정
				
				if(result == 0) { // 중복이 아니라면 반복문 종료
			...
			while(true) {
				...
				if(memberPw.equals(memberPw2)) { // 일치하면 멈춤
					 break;
				}else {
			...
			int result = service.singUp(signUpMember);
	...
	public Member login() {
		...
		mem.setMemberId(memberId); // setter를 이용해서 초기화
		mem.setMemberPw(memberPw); // setter를 이용해서 초기화
	...
	public void selectAll() {
		...
		System.out.println("[가입된 회원의 목록 조회]");
		...
		try {
			...
			if(memberList.isEmpty()) { // 비어있다 == 조회 결과 없음
