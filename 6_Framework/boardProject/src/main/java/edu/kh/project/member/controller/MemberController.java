...
@Controller // 요청, 응답 처리하는 클래스 + bean 등록(Spring이 관리하는 객체)
...
// ex) 여러개인 경우 @SessionAttributes({"loginMember"},{"adminMember"})
public class MemberController {
	...
	public String login(/*@RequestParam("inputEmail")*/ String inputEmail,
		...
		// ** 파라미터의 name 속성 값과
		//    매개 변수 명이 같으면 @RequestParam 생략 가능!! **
	...
	public String login(Member inputMember, Model model
		...
		// -> 기본적으로 request scope
		// -> @SessionAttributes 어노테이션과 함께 사용 시 Session scope가 된다.
		
		// @RequestParam(value="savaId") String saveId
		// -> name 속성 값이 saveId인 파라미터를 전달 받아 저장
		// -> required=false : 필수 아님 (null 허용)
		// (주의) required 속성 미작성 시 기본 값은 true
		// -> 파라미터가 전달되지 않는 경우 주의
		
		// HttpServletResponse resp : 서버 -> 클라이언트 응답 방법을 가진 객체
		...
		if(loginMember != null) {// 로그인 성공 시
			...
			// 클라이언트가 어떤 요청을 할 때 쿠기가 첨부될지 경로(주소)를 지정
	...
	public String signUp(Member inputMember
		...
		// 리 다이렉트 시 session에 잠깐 올라갔다 내려오도록 세팅
