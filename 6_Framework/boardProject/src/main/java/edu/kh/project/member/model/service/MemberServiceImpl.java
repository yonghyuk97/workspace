...
@Service // Service Layer
		 // 비즈니스 로직(데이터 가공, dao 호출, 트랜젝션 제어)처리하는 클래스라고 명시
		 // + Bean으로 등록하는 어노테이션
public class MemberServiceImpl implements MemberService{
	...
	//    -> 객체를 직접 만들지 않고 스프링이 만든걸 주입함(Spring에 의존)
	
	@Autowired // 다른 곳에서 만든 객체를 자동으로 연결해준다.
	...
	// 예외가 발생하면 rollback 하겠다
	// 발생 안하면 Service 종료시 알아서 commit 
	...
	public int signUp(Member inputMember) {
		...
		// 비밀번호를 BCrypt를 이용하여 암호화한 후 다시 inputMember에 세팅
